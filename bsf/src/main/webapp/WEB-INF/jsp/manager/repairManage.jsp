            ...
            <div class="modal-body">
                <form class="form-horizontal" id="detail_modal_form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <p class="form-control-static" id="detail_modal_repairman_username"></p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-4 control-label">维修工邮箱</label>
                        <div class="col-sm-8">
                            <p class="form-control-static" id="detail_modal_repairman_email"></p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-4 control-label">维修工电话</label>
                        <div class="col-sm-8">
                            <p class="form-control-static" id="detail_modal_repairman_phone"></p>
                        </div>
                    </div>
                </form>
            </div>
...
<div class="modal fade" tabindex="-1" role="dialog" id="update_modal">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">修改维修信息</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal" id="update_modal_form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select class="form-control" id="update_modal_locate_select">
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select class="form-control" id="update_modal_repairType_select">
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <textarea class="form-control" rows="3" id="update_modal_detailInf_textarea"></textarea>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <button type="button" class="btn btn-primary" style="float:right;" id="add_repair_btn"><i class="glyphicon glyphicon-plus"></i> 新增</button>
                    <br>
                    <hr style="clear:both;">
...
<script type="text/javascript">
    ...
    $("#update_modal_campus_select").change(function(){
        ...
        typeBind(type_selector,campus,-1);
        var locate_selector=$("#update_modal_locate_select");
        locateBind(locate_selector,-1,-1);
    });

    $("#update_modal_type_select").change(function(){
            ...
            success:function (result) {
                ...
                if(result.success){
                    ...
                    $.each(repairs,function (i,repair) {
                        ...
                        tbodyInf+='<button type="button" idClass= "'+repair.id+'" pageNumber="'+pageNumber+'" class="btn btn-primary btn-xs update_btn"><i class=" glyphicon glyphicon-pencil"></i></button>';
        ...
        $.ajax({
            ...
            success:function (result) {
                if(result.success==true){
                    ...
                    $("#detail_modal_repairman_username").html(repair.repairman.username);

                    $("#detail_modal_repairman_email").html(repair.repairman.email);

                    $("#detail_modal_repairman_phone").html(repair.repairman.phone);
    ...
    $(document).on("click",".update_btn",function (){
        ...
        var repairId= $(this).attr("idClass");
        $("#update_modal_update_btn").attr("idClass",repairId);

        $.ajax({
            type:"get",
            url:"${APP_PATH}/repair/getByIdNoAdress",
            data:{
                id:repairId
            },
            success:function (result) {
                if(result.success==true){
                   // $("#update_modal").modal('show');
                    ...
                    var repair=result.data;
                    var campus=repair.adress.campus;
                    var type=repair.adress.type;
                    var locate=repair.adress.id;
                    var detailInf=repair.detailInf;
                    var repairTypeId=repair.type.id;
                    var campus_selector= $("#update_modal_campus_select");
                    var type_selector=$("#update_modal_type_select");
                    var locate_selector=$("#update_modal_locate_select");
                    var repairType_selector=$("#update_modal_repairType_select");
                    campusBind(campus_selector,campus);
                    typeBind(type_selector,campus,type);
                    locateBind(locate_selector,type,locate);
                    repairType(repairType_selector,repairTypeId);

                    $("#update_modal_detailInf_textarea").val(detailInf);
                    $("#update_modal").modal('show');
    ...
    $("#update_modal_update_btn").click(function () {
        ...
        //准备好数据
        var adressId=$("#update_modal_locate_select").val();
        var detailInf=$("#update_modal_detailInf_textarea").val();
        var typeId=$("#update_modal_type_select").val();
        var userId=${userInf.id};
        if(adressId==null){
            layer.msg('请选择地址信息',{time:3000,icon:5,shift:6},function () {});
            return;
        ...
        $.ajax({
            ...
            success:function (result) {
                if(result.success==true){
                    $.ajax({
                        url:"${APP_PATH}/repair/updateRepair",
                        type:"post",
                        data:{
                            id:id,
                            adressId:adressId,
